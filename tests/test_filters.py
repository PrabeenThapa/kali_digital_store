...
from apps.telegram_bot.filters.permissions import ValidAmountFilter, HasPermissionFilter
...
class TestHasPermissionFilter:
    ...
    async def test_user_has_permission(self):
        ...
        with patch('apps.telegram_bot.filters.permissions.check_role_cached', new_callable=AsyncMock, return_value=Permission.USE):
    ...
    async def test_user_with_multiple_permissions(self):
        ...
        with patch('apps.telegram_bot.filters.permissions.check_role_cached', new_callable=AsyncMock, return_value=combined):
            result = await f(event)
        assert result is True
    ...
    async def test_user_no_role(self):
        ...
        with patch('apps.telegram_bot.filters.permissions.check_role_cached', new_callable=AsyncMock, return_value=None):
            result = await f(event)
        assert result is False
    ...
    async def test_owner_has_all_permissions(self):
        ...
        with patch('apps.telegram_bot.filters.permissions.check_role_cached', new_callable=AsyncMock, return_value=all_perms):
            result = await f(event)
        assert result is True
