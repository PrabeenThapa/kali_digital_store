...
# Currencies without minor units (no cents/pence)
ZERO_DEC_CURRENCIES = {"JPY", "KRW"}


def currency_to_stars(amount_rub: int) -> int:
    """Convert a currency amount to Telegram Stars (always rounded up)."""
...
async def send_stars_invoice(
    ...
    """Send a Telegram Stars invoice (currency='XTR', provider_token='')."""
    stars = currency_to_stars(amount)
    prices = [LabeledPrice(label=localize("payments.invoice.label.stars", stars=stars), amount=stars)]
    payload = {"op": "topup_balance_stars", "amount_rub": int(amount), "stars": stars}
    if payload_extra:
    ...
    await bot.send_invoice(
        ...
        description=description or localize(
            "payments.invoice.desc.topup.stars", amount=int(amount), currency=EnvKeys.PAY_CURRENCY
        ),
...
def _minor_units_for(currency: str) -> int:
    """Return the multiplier that converts major to minor units."""
...
async def send_fiat_invoice(
    *,
    ...
    """Send a fiat-provider Telegram Payments invoice."""
    ...
    amount_minor = int(amount) * _minor_units_for(currency)
    prices = [
        LabeledPrice(
...
class CryptoPayAPIError(Exception):
    """Raised when the CryptoPay API returns an error response."""
...
class CryptoPayAPI:
    """Minimal async client for the CryptoPay Bot API."""
    ...
    async def create_invoice(
        ...
        """Create a CryptoPay fiat invoice."""
        params = {
            "currency_type": "fiat",
            "fiat": currency,
            "amount": str(amount),
