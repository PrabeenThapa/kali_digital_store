...
from packages.database.methods.read import invalidate_item_cache, invalidate_category_cache
from packages.database.methods.cache_utils import safe_create_task
from packages.database.models import Database, Goods, ItemValues, Categories, Role, User
...
async def delete_item_from_position(item_id: int) -> None:
    ...
    item_name = None
    async with Database().session() as s:
        item_name = (await s.execute(
            select(Goods.name)
            .join(ItemValues, ItemValues.item_id == Goods.id)
            .where(ItemValues.id == item_id)
        )).scalar()
...
async def delete_category(category_name: str) -> None:
    """Delete a category and all products/stock inside it, including reseller products and orphan goods."""
    from packages.database.models.main import ResellerProduct
    async with Database().session() as s:
        # 1. Delete from Categories table if exists
        ...
        # 2. Delete local Goods associated with this category by name or category_id
        local_goods = (await s.execute(select(Goods))).scalars().all()
        from apps.telegram_bot.utils.category_resolver import resolve_category
        for g in local_goods:
            if resolve_category(g.name) == category_name:
                await s.delete(g)

        # 3. Disable all ResellerProducts that belong to this category so they don't recreate it
