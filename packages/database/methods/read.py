...
from packages.database.models import Database, User, ItemValues, Goods, Categories, Role, BoughtGoods, \
    Operations, ReferralEarnings, Permission
from packages.database.models.main import PromoCodes, PromoCodeUsages, CartItems, Reviews
...
async def get_item_info(item_name: str) -> dict | None:
    ...
    async with Database().session() as s:
        result = await s.execute(
            select(Goods, Categories.icon_custom_emoji_id)
        ...
        if not row:
            return None
        obj, cat_emoji = row
        data = _obj_to_dict(obj, Goods)
        data['cat_icon_custom_emoji_id'] = cat_emoji
        return data
...
async def get_financial_report() -> dict:
    """
    Returns a dictionary containing total deposits, current balances, total revenue, total costs, and net profit.
    """
    from packages.database.models.main import Payments, Operations, User, BoughtGoods, PaymentStatus
    async with Database().session() as s:
        # 1. Total Deposits (Completed payments)
        payments_query = select(func.sum(Payments.amount)).where(Payments.status == PaymentStatus.SUCCEEDED)
        total_payments = (await s.execute(payments_query)).scalar() or Decimal("0")

        # 2. Current Balances
        balance_query = select(func.sum(User.balance))
        total_balances = (await s.execute(balance_query)).scalar() or Decimal("0")

        # 3. Revenue (Sum of price in BoughtGoods)
        revenue_query = select(func.sum(BoughtGoods.price))
        total_revenue = (await s.execute(revenue_query)).scalar() or Decimal("0")

        # 4. Cost (Sum of cost_price in BoughtGoods)
        cost_query = select(func.sum(BoughtGoods.cost_price))
        total_cost = (await s.execute(cost_query)).scalar() or Decimal("0")
