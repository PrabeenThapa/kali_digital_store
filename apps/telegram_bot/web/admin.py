...
from packages.database.models.main import (
    ...
    ResellerSource, ResellerProduct,
)
...
def _format_product_price(model, _name):
    mode = "custom" if model.sell_price is not None else "auto"
    color = "#7c3aed" if mode == "custom" else "#64748b"
    return Markup(
        f'<strong>${model.effective_sell_price:.2f}</strong> '
        f'<span style="color:{color};font-size:11px">{mode}</span>'
    )


def _format_product_status(model, _name):
    if model.is_enabled:
        return Markup('<span style="color:#15803d;font-weight:600">● Live</span>')
    return Markup('<span style="color:#b91c1c;font-weight:600">● Paused</span>')


def _format_product_stock(model, _name):
    return "Unlimited / preorder" if model.stock is None else str(model.stock)


class ResellerProductAdmin(AuditModelView, model=ResellerProduct):
    column_list = [
        ResellerProduct.id, ResellerProduct.source_id, ResellerProduct.name,
        ResellerProduct.cost_price, ResellerProduct.sell_price,
        ResellerProduct.is_enabled, ResellerProduct.is_featured,
        ResellerProduct.stock, ResellerProduct.last_synced,
    ]
    column_searchable_list = [ResellerProduct.name, ResellerProduct.external_id]
    ...
    column_default_sort = (ResellerProduct.last_synced, True)
    column_labels = {
        "source_id": "Source",
    ...
    column_formatters = {
        "sell_price": _format_product_price,
        "is_enabled": _format_product_status,
        "stock": _format_product_stock,
    }
    column_formatters_detail = column_formatters
    form_columns = [
        ResellerProduct.sell_price, ResellerProduct.markup_percent,
        ResellerProduct.is_enabled, ResellerProduct.is_featured,
        ResellerProduct.category_override,
    ]
    form_args = {
        "sell_price": {"description": "Leave empty to calculate from wholesale cost and markup."},
        "markup_percent": {"description": "Used only while the sell price is empty."},
        "is_featured": {"description": "Show this product in the ⭐ Featured section of the shop."},
        "category_override": {"description": "Leave empty to derive the category from the product name."},
    }
    can_create = False
    can_delete = False
    ...
    category = "Reseller Catalog"


class ResellerSourceAdmin(AuditModelView, model=ResellerSource):
    column_list = [ResellerSource.id, ResellerSource.name, ResellerSource.base_url,
                   ResellerSource.is_active, ResellerSource.last_synced, ResellerSource.created_at]
    column_searchable_list = [ResellerSource.name]
    column_sortable_list = [ResellerSource.id, ResellerSource.name, ResellerSource.last_synced]
    column_default_sort = (ResellerSource.id, True)
    form_columns = [ResellerSource.name, ResellerSource.base_url,
                    ResellerSource.api_key, ResellerSource.is_active]
    column_details_exclude_list = ["api_key", "products"]
    name = "API Source"
    name_plural = "API Sources"
...
def create_admin_app() -> Starlette:
    ...
    from apps.telegram_bot.web.export import export_routes
    ...
    admin.add_view(ResellerProductAdmin)
    admin.add_view(ResellerSourceAdmin)
