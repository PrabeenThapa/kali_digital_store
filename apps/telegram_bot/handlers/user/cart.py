...
from packages.database.methods.create import add_to_cart
from packages.database.methods.read import get_cart_items, get_cart_count
from packages.database.methods.delete import remove_from_cart, clear_cart
from packages.database.methods.transactions import checkout_cart_transaction
...
async def _resolve_promo_price(price: Decimal, promo_code: str | None) -> Decimal | None:
    ...
    if not promo_code:
        return None
    from packages.database.methods.read import get_promo_code
    promo = await get_promo_code(promo_code)
    if not promo or not promo.get('is_active'):
        return None
    return apply_promo_discount(price, promo)
