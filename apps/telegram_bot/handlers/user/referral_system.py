...
from apps.telegram_bot.handlers.other import get_bot_info
from apps.telegram_bot.keyboards import back, referral_system_keyboard, lazy_paginated_keyboard
...
async def referral_callback_handler(call: CallbackQuery, state: FSMContext):
    ...
    try:
        signup_bonus = float(EnvKeys.REFERRAL_SIGNUP_BONUS or 0)
    ...
    text = (
        ...
        f"{localize('referral.description', percent=referral_percent, signup_bonus=signup_bonus)}"
    )
