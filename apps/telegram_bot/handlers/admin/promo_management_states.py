...
from packages.database.methods.create import create_promo_code
from packages.database.methods.delete import delete_promo_code
from packages.database.methods.update import toggle_promo_code
from packages.database.methods.lazy_queries import query_promo_codes
from packages.database.methods.read import get_promo_code, check_category, get_item_info
...
from apps.telegram_bot.states import PromoFSM
...
async def promo_management_handler(call: CallbackQuery, state: FSMContext):
    ...
    # Rebuild: add Create, Quick Generate, then Back
    ...
    kb.row(InlineKeyboardButton(text="Quick Generate", callback_data="promo_quickgen"))
...
async def _show_promo_view(message, promo_id: int):
    ...
    buttons = [
        ...
        ("Remove Promo", f"promo_d_{promo_id}"),
...
async def promo_receive_code(message: Message, state: FSMContext):
    ...
    buttons = [
        ...
        ("Account Upgrade (% off all)", "promo_type_account_upgrade"),
        (localize("btn.back"), "promo_mgmt"),
    ]
...
async def promo_receive_type(call: CallbackQuery, state: FSMContext):
    dtype = call.data.split("_")[2]  # "percent", "fixed", "balance", or "account"
    if dtype == "account":  # promo_type_account_upgrade → split gives 'account'
        dtype = "account_upgrade"
    await state.update_data(promo_type=dtype)
    if dtype == "balance":
        type_label = localize("admin.promo.type.balance")
    elif dtype == "account_upgrade":
        type_label = "% account discount"
    elif dtype == "percent":
        type_label = "%"
    else:
...
async def promo_receive_value(message: Message, state: FSMContext):
    ...
    try:
        ...
        if value <= 0:
            raise ValueError
        if data.get('promo_type') in ('percent', 'account_upgrade') and value > 100:
            raise ValueError
...
async def promo_receive_max_uses(message: Message, state: FSMContext):
    ...
    await message.answer(
        "<b>Per-User Limit</b>\n\nSpecify max uses per individual user (1 = once per user, N = up to N uses, 0 = unlimited per user):",
        parse_mode="HTML",
        reply_markup=back("promo_mgmt"),
    )
    await state.set_state(PromoFSM.waiting_max_uses_per_user)


@router.message(PromoFSM.waiting_max_uses_per_user, F.text)
async def promo_receive_max_uses_per_user(message: Message, state: FSMContext):
    try:
        max_user_uses = int(message.text.strip())
        if max_user_uses < 0:
            raise ValueError
    except (ValueError, TypeError):
        await message.answer(localize("admin.promo.invalid_value"), reply_markup=back("promo_mgmt"))
    ...
    await state.update_data(promo_max_uses_per_user=max_user_uses)

    data = await state.get_data()
    if data.get('is_quickgen'):
        # Auto-generate kali code, skip expiry
        code = _gen_kali_code()
        await state.update_data(promo_code=code, promo_expires=None)
...
async def promo_receive_expires(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    if data.get('promo_type') in ('balance', 'account_upgrade'):
        # Balance/account promos have no item/category binding — create immediately
        await _finalize_promo_creation(message, state, message.from_user.id)
        return
...
async def _finalize_promo_creation(target, state: FSMContext, user_id: int):
    ...
    promo_id = await create_promo_code(
        ...
        max_uses_per_user=data.get('promo_max_uses_per_user', 1),
...
def _gen_kali_code() -> str:
    """Generate KALI + 5 random uppercase alphanumeric characters."""
    chars = string.ascii_uppercase + string.digits
    return "KALI" + "".join(random.choices(chars, k=5))


@router.callback_query(F.data == "promo_quickgen", HasPermissionFilter(permission=Permission.PROMO_MANAGE))
async def promo_quickgen_start(call: CallbackQuery, state: FSMContext):
    """Quick Generate: pick type → enter value → enter max_uses → code auto-created."""
    buttons = [
        ("Balance Top-Up (credits USDT)", "qgen_type_balance"),
        ("Account Upgrade (% off all purchases)", "qgen_type_account_upgrade"),
        (localize("btn.back"), "promo_mgmt"),
    ]
    await call.message.edit_text(
        "<b>Quick Generate Promo</b>\n\nSelect the promo type:",
        parse_mode="HTML",
        reply_markup=simple_buttons(buttons),
    )
    await state.set_state(PromoFSM.waiting_type)
    await state.update_data(is_quickgen=True)


@router.callback_query(F.data.startswith("qgen_type_"), PromoFSM.waiting_type)
async def qgen_type_chosen(call: CallbackQuery, state: FSMContext):
    dtype = call.data.replace("qgen_type_", "")  # "balance" or "account_upgrade"
    await state.update_data(promo_type=dtype, is_quickgen=True)
    hint = "USDT amount to credit" if dtype == "balance" else "discount % (e.g. 10 for 10% off)"
    await call.message.edit_text(
        f"<b>Quick Generate</b>\n\nEnter the value ({hint}):",
        parse_mode="HTML",
        reply_markup=back("promo_mgmt"),
    )
    await state.set_state(PromoFSM.waiting_value)
