...
from apps.telegram_bot.keyboards import back, close, simple_buttons, lazy_paginated_keyboard
from packages.database.methods.audit import log_audit
from apps.telegram_bot.filters import HasPermissionFilter
from apps.telegram_bot.states import UserMgmtStates


from packages.config.config import EnvKeys
from apps.telegram_bot.utils import LazyPaginator, validate_telegram_id, validate_money_amount, UserDataUpdate
...
async def _build_user_profile(bot, target_id: int, caller_perms: int = 0):
    ...
    discount_pct = float(user.get('discount_percent', 0)) if user.get('discount_percent') else 0

    if caller_perms & Permission.PROMO_MANAGE:
        actions.append(("🏷️ Set Account Discount %", f"set-user-discount_{target_id}"))
        if discount_pct > 0:
            actions.append(("❌ Clear Account Discount", f"clear-user-discount_{target_id}"))
        actions.append(("🎁 View Used Promo Codes", f"view-user-promos_{target_id}"))
    ...
    if discount_pct > 0:
        lines.append(f"🏷️ <b>Account Discount:</b> {discount_pct:g}% off all purchases")
...
# ── User Discount Management & Promo Usage Log ────────────────────────────────

@router.callback_query(F.data.startswith('set-user-discount_'), HasPermissionFilter(Permission.PROMO_MANAGE))
async def prompt_set_user_discount(call: CallbackQuery, state: FSMContext):
    target_id = int(call.data.split('_')[1])
    await state.update_data(target_discount_user_id=target_id)
    await call.message.edit_text(
        f"🏷️ <b>Set Account Discount</b>\n\nEnter discount percentage for user <code>{target_id}</code> (1 to 100, or 0 to clear):",
        parse_mode="HTML",
        reply_markup=back(f"check-user_{target_id}")
    )
    await state.set_state(UserMgmtStates.waiting_user_discount)


@router.message(UserMgmtStates.waiting_user_discount, F.text)
async def receive_user_discount(message: Message, state: FSMContext):
    data = await state.get_data()
    target_id = data.get('target_discount_user_id')
    if not target_id:
        await state.clear()
        return
    ...
    try:
        ...
        await message.answer("❌ Invalid percentage. Enter a number between 0 and 100:", reply_markup=back(f"check-user_{target_id}"))
        return

    from packages.database.methods import update_user_discount
    await update_user_discount(target_id, val)
    await log_audit("user_discount_updated", user_id=message.from_user.id, resource_type="User", resource_id=str(target_id), details=f"discount={val}%")
    await state.clear()
    ...
    if result:
        text, markup = result
        await message.answer(f"✅ Discount updated to <b>{val:g}%</b>!\n\n" + text, parse_mode='HTML', reply_markup=markup)


@router.callback_query(F.data.startswith('clear-user-discount_'), HasPermissionFilter(Permission.PROMO_MANAGE))
async def clear_user_discount_handler(call: CallbackQuery):
    target_id = int(call.data.split('_')[1])
    from packages.database.methods import update_user_discount
    await update_user_discount(target_id, 0)
    await log_audit("user_discount_cleared", user_id=call.from_user.id, resource_type="User", resource_id=str(target_id))
    await call.answer("Discount removed!")
...
@router.callback_query(F.data.startswith('view-user-promos_'), HasPermissionFilter(Permission.PROMO_MANAGE))
async def view_user_promos_handler(call: CallbackQuery):
    target_id = int(call.data.split('_')[1])
    from packages.database.models.main import PromoCodeUsages, PromoCodes
    from packages.database import Database
    from sqlalchemy import select

    async with Database().session() as s:
        stmt = (
            select(PromoCodeUsages, PromoCodes)
            .join(PromoCodes, PromoCodeUsages.promo_id == PromoCodes.id)
            .where(PromoCodeUsages.user_id == target_id)
            .order_by(PromoCodeUsages.used_at.desc())
        )
        rows = (await s.execute(stmt)).all()

    if not rows:
        await call.message.edit_text(
            f"🎁 <b>Promo Code History for User {target_id}</b>\n\n<i>No promo codes used by this user yet.</i>",
            parse_mode="HTML",
            reply_markup=back(f"check-user_{target_id}")
        )
        return

    lines = [f"🎁 <b>Promo Code History for User {target_id}</b> ({len(rows)} usages):\n"]
    for idx, (usage, promo) in enumerate(rows, 1):
        dt_str = usage.used_at.strftime("%Y-%m-%d %H:%M UTC") if usage.used_at else "N/A"
        lines.append(f"{idx}. <b>{promo.code}</b> ({promo.discount_type}: {promo.discount_value:g}) — <i>{dt_str}</i>")

    text = "\n".join(lines)
    await call.message.edit_text(text, parse_mode="HTML", reply_markup=back(f"check-user_{target_id}"))
